<template>
  <Dialog :title="dialogTitle" v-model="dialogVisible" width="92%" top="5vh">
    <div v-loading="formLoading" class="stockin-form">
      <el-row :gutter="12" class="stockin-form__row">
        <!-- 左侧：选择入库商品 -->
        <el-col :span="12" class="stockin-form__col">
          <div class="panel">
            <div class="panel__title">选择入库商品</div>
            <div class="panel__body">
              ...
              >
                <el-row :gutter="10">
                  ...
                  <el-col :span="12">
                    <el-form-item label="入库单价:" prop="inboundPrice" required>
                      <el-input v-model="itemForm.inboundPrice" placeholder="请输入入库单价">
                        <template #append>元</template>
                      </el-input>
                    </el-form-item>
                  </el-col>
                  <el-col :span="12">
                    <el-form-item label="入库数量:" prop="stockinNums" required>
                      <el-input v-model="itemForm.stockinNums" placeholder="请输入入库数量">
              ...
              >
                <el-table-column type="expand">
                  <template #default="{ row }">
                    <div class="sku-panel">
                      <div class="sku-panel__tip">请选择下方规格后入库</div>
                      <el-table
                      ...
                      >
                        ...
                        <!-- <el-table-column label="成本价" align="center" width="80">
                          <template #default="{ row: sku }">
                            {{ sku.cost ?? '-' }}
                          </template>
                        </el-table-column> -->
            ...
            <div class="panel__body">
              ...
              >
                <el-form-item label="供应商:" prop="stockSupplierId">
                  <el-select
                    v-model="formData.stockSupplierId"
                    placeholder="请选择供应商"
                    filterable
                    clearable
                    class="!w-full"
          ...
          <div class="panel panel--flex">
            ...
            <div class="panel__body panel__body--table">
              ...
              >
                ...
                <el-table-column label="入库单价" align="center" prop="inboundPrice" width="90" />
                <el-table-column label="实收金额" align="center" prop="amount" width="90" />
                <el-table-column label="备注" align="center" prop="stockinGoodsRemark"  />
...
const formData = ref({
  ...
  inboundTime: undefined as number | string | undefined,
  stockinTotalNums: undefined as number | undefined,
  totalAmount: undefined as number | undefined,
  remark: undefined as string | undefined,
  isAudit: undefined as number | undefined,
  auditTime: undefined as number | undefined,
  isSettle: undefined as number | undefined,
  settleTime: undefined as number | undefined
})

const formRules = reactive({
  stockSupplierId: [{ required: true, message: '请选择供应商', trigger: 'change' }],
  inboundTime: [{ required: true, message: '请选择入库日期', trigger: 'change' }]
})
const formRef = ref()
...
const itemForm = ref({
  ...
  spec: '',
  unitName: '',
  inboundPrice: '' as string | number,
  stockinNums: 1 as string | number,
  remark: ''
})
const itemFormRules = reactive({
  inboundPrice: [{ required: true, message: '请输入入库单价', trigger: 'blur' }],
  stockinNums: [{ required: true, message: '请输入入库数量', trigger: 'blur' }]
})
...
const getSupplierList = async () => {
  const data = await SupplierApi.getSupplierPage({ pageNo: 1, pageSize: 100 })
...
const handleSkuSelect = (product: StoreProductVO, sku: StoreProductAttrValueVO) => {
  ...
  itemForm.value = {
    ...
    inboundPrice: sku.price  ?? product.price ?? 0,
    stockinNums: 1,
    remark: ''
  }
...
const handleBarcodeEnter = async () => {
  ...
  try {
    ...
    if (matchedProduct && matchedSku) {
      ...
      expandRowKeys.value = [matchedProduct.id]
      message.warning('请展开商品并选择具体规格后再入库')
    } else {
...
const editDetail = (index: number) => {
  ...
  itemForm.value = {
    ...
    remark: item.stockinGoodsRemark || ''
  }
...
const confirmAddItem = async () => {
  ...
  if (!itemForm.value.stockGoodsId || !itemForm.value.stockGoodsValueId) {
    message.warning('请先展开商品并选择一个规格后再入库')
    return
  }
  const nums = Number(itemForm.value.stockinNums)
  const price = Number(itemForm.value.inboundPrice)
  if (!nums || nums <= 0) {
    message.warning('入库数量必须大于0')
    return
  }
  if (price < 0 || Number.isNaN(price)) {
    message.warning('请输入正确的入库单价')
    return
  }
  ...
  const detail: DetailItem = {
    ...
    inboundPrice: price,
    amount,
    stockinGoodsRemark: itemForm.value.remark,
...
const removeDetail = (index: number) => {
  ...
  // 若删除的是正在编辑的行，或编辑下标受影响，则清空左侧编辑状态
...
const submitForm = async () => {
  ...
  if (!detailList.value.length) {
    message.warning('请至少添加一件入库商品')
    return
  }
  const invalid = detailList.value.some((item) => !item.stockGoodsValueId)
  if (invalid) {
    message.warning('入库明细存在未选择规格的商品，请检查')
    return
  }
  formLoading.value = true
  try {
    const totalNums = detailList.value.reduce((sum, item) => sum + Number(item.stockinNums || 0), 0)
    const data = {
      ...formData.value,
      stockinTotalNums: totalNums,
      totalAmount: totalAmountDisplay.value,
      stockinLists: detailList.value.map((item) => ({
        stockGoodsId: item.stockGoodsId,
        stockGoodsValueId: item.stockGoodsValueId,
        stockinNums: item.stockinNums,
        inboundPrice: item.inboundPrice,
