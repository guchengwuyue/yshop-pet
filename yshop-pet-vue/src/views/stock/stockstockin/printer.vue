<template>
  <Dialog title="入库单打印" v-model="dialogVisible" width="860px" top="5vh">
    <div v-loading="formLoading" class="printer-wrap">
      ...
      <div id="stockin-print-area" class="print-sheet">
        <h1 class="print-sheet__title">入库单</h1>
        ...
        <table class="print-table">
          ...
          <tbody>
            <tr v-for="(item, index) in detailList" :key="index">
              ...
              <td>{{ item.spec || '' }}</td>
              <td>{{ item.unitName || '' }}</td>
              <td>{{ formatQty(item.stockinNums) }}</td>
              <td>{{ formatMoney(item.inboundPrice) }}</td>
              <td>{{ formatMoney(item.amount) }}</td>
              <td>{{ item.stockinGoodsRemark || '' }}</td>
            </tr>
...
/** 商品入库 打印预览 */
defineOptions({ name: 'StockinPrinter' })
...
const pageQty = computed(() =>
  detailList.value.reduce((sum, item) => sum + Number(item.stockinNums || 0), 0)
)
...
const totalAmount = computed(() => {
  if (formData.value.totalAmount !== undefined && formData.value.totalAmount !== null) {
    return Number(formData.value.totalAmount)
  }
...
#stockin-print-area .print-sheet__title {
  margin: 0 0 16px;
  text-align: center;
  font-size: 28px;
  font-weight: 700;
...
#stockin-print-area .print-sheet__meta {
  display: flex;
  justify-content: space-between;
  align-items: flex-start;
  margin-bottom: 10px;
...
#stockin-print-area .print-sheet__meta-left,
#stockin-print-area .print-sheet__meta-center {
  flex: 1;
}

#stockin-print-area .print-sheet__meta-right {
  flex: 1;
  text-align: right;
  line-height: 1.6;
}

#stockin-print-area .print-table {
  width: 100%;
  border-collapse: collapse;
  table-layout: fixed;
  font-size: 13px;
...
#stockin-print-area .print-table th,
#stockin-print-area .print-table td {
  border: 1px solid #000;
  padding: 6px 4px;
  text-align: center;
  word-break: break-all;
...
#stockin-print-area .print-table .sum-label {
  text-align: left;
  padding-left: 12px;
}

#stockin-print-area .print-sheet__footer {
  display: flex;
  justify-content: space-between;
  margin-top: 28px;
  padding: 0 8px;
