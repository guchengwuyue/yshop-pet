  ...
  <Dialog :title="dialogTitle" append-to-body="true" v-model="dialogVisible" width="70%">
    ...
    <el-tabs v-model="activeName" type="border-card" @tab-click="handleClick">
      <el-tab-pane label="基本信息" name="one">
        ...
        <el-form-item label="启动库存预警" prop="inventoryWarningEnable">
          <el-radio-group v-model="formValidate.inventoryWarningEnable">
            <el-radio :label="1">是</el-radio>
            <el-radio :label="0">否</el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item label="库存预警数量" prop="inventoryWarningNumber">
          <el-input-number v-model="formValidate.inventoryWarningNumber" class="input-width"  />
        </el-form-item>
            ...
            <el-col :xl="24" :lg="24" :md="24" :sm="24" :xs="24" v-if="manyFormValidate.length">
              ...
              <el-col :span="24">
                <el-form-item label="商品属性：" class="labeltop">
                  <el-table :data="manyFormValidate" size="small" >
                    <el-table-column type="myindex" v-for="(item,index) in formValidate.header" :key="index" :label="item.title" :property="item.slot" align="center" :width="item.slot == 'good_code' || item.slot == 'bar_code'  ? 180 : ''">
                      <template #default="scope">
                        ...
                        <div v-else-if="item.slot == 'stock'" align="center" >
                          {{ scope.row[item.slot] }}
                        </div>
                        <div v-else-if="item.slot == 'good_code'"  >
                          <el-input  v-model="scope.row.good_code" >
                            <template #append>
                              <el-button  @click="scope.row.good_code = generateStockCode('SP')">生成</el-button>
                            </template>
                          </el-input>
                        </div>
                        <div v-else-if="item.slot == 'bar_code'"  >
                          <el-input  v-model="scope.row.bar_code" >
                            <template #append>
                              <el-button  @click="scope.row.bar_code = generateStockCode()">生成</el-button>
                            </template>
                          </el-input>
                        </div>
                        <div v-else >
                          <el-input  v-model="scope.row[item.slot]"  />
                        </div>
                      </template>
                    </el-table-column>
          ...
          <el-col :xl="23" :lg="24" :md="24" :sm="24" :xs="24" v-if="formValidate.spec_type === 0">
            <el-form-item >
              <el-table :data="oneFormValidate"  size="small" style="width: 90%;">
                ...
                <el-table-column prop="good_code" label="商品编号" align="center" width="180">
                  <template #default="scope">
                    <el-input type="text" v-model="scope.row.good_code">
                      <template #append>
                        <el-button  @click="scope.row.good_code = generateStockCode('SP')">生成</el-button>
                      </template>
                    </el-input>
                ...
                <el-table-column prop="bar_code" label="商品条码" align="center" width="160">
                  <template #default="scope">
...
import { generateStockCode } from '@/utils'
...
const formValidate = ref({
    ...
    otPrice: 0,
    stock: 0,
    inventoryWarningNumber: 0,
    inventoryWarningEnable: 0,
    is_new: 0,
    postage: 0,
    is_postage: 0,
    is_sub: 0,
...
const formRules = reactive({
  unit_name: [{ required: true, message: '商品单位不能为空', trigger: 'blur' }],
...
const open = async (type: string, id?: number) => {
  ...
  resetForm()
  console.log('formValidate2.value:',formValidate.value)
  // 修改时，设置数据
  if (id) {
