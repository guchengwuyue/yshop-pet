<template>
    ...
    <ContentWrap>
      ...
      >
        ...
        <el-form-item>
          ...
          <!-- 批量导入按钮 -->
          <el-button type="primary" @click="handleBatchImport"><Icon icon="ep:upload" class="mr-5px" /> 批量导入</el-button>
        </el-form-item>
      </el-form>
    </ContentWrap>
        ...
        <el-tab-pane v-for="item in tableTabs" :label="item.title" :key="item.name" :name="item.name">
          <el-table v-loading="loading" :data="list"> 
            ...
            <el-table-column label="商品名称" align="center" prop="storeName">
              <template #default="scope">
                <span class="text-center">{{ scope.row.storeName }}</span>
                <el-tag>{{ scope.row.specType == 1 ? '多规格' : '单规格' }}</el-tag>
              </template>
            </el-table-column>
            <el-table-column label="商品价格" align="center" prop="price" />
            <el-table-column label="库存" align="center"  width="80" prop="stock" />
            <el-table-column label="启用库存预警" align="center" prop="inventoryWarningEnable" >
              <template #default="scope">
                <el-switch v-model="scope.row.inventoryWarningEnable" @change="onSale(scope.row.id,scope.row.inventoryWarningEnable)" :active-value="1" :inactive-value="0" />
               
              </template>
            </el-table-column>
            <el-table-column label="库存预警数量" align="center"  width="110" prop="inventoryWarningNumber" />
            <el-table-column
              label="添加时间"
              align="center"
              prop="createTime"
  ...
  <!-- 批量导入弹窗 -->
  <StoreProductImport ref="importRef" @success="getList" />
</template>
...
import StoreProductImport from './import.vue'
...
// /** 启用库存预警按钮操作 */
// const onInventoryWarningEnableChange = async (id,inventoryWarningEnable) => {
//   await StoreProductApi.updateStoreProduct(id,inventoryWarningEnable)
//   message.success('操作成功')
//   await getList()
// }
