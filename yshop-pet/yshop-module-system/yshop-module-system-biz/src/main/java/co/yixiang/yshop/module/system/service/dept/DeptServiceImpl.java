...
public class DeptServiceImpl implements DeptService {
    ...
    public List<DeptDO> getDeptList(DeptListReqVO reqVO) {
        if(reqVO.getIsAccess() != null && reqVO.getIsAccess()){
            //获取部门子集id
            AdminUserDO adminUserDO = userMapper.selectById(getLoginUserId());
            List<DeptDO> childDeptList = getChildDeptList(adminUserDO.getDeptId());

            List<Long> ids = childDeptList.stream().map(DeptDO::getId).collect(Collectors.toList());
            ids.add(adminUserDO.getDeptId());
