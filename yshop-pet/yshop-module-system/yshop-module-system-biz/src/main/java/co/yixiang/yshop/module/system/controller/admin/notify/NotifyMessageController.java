package co.yixiang.yshop.module.system.controller.admin.notify;
...
public class NotifyMessageController {
    ...
    public CommonResult<PageResult<NotifyMessageRespVO>> getMyMyNotifyMessagePage(@Valid NotifyMessageMyPageReqVO pageVO) {
        PageResult<NotifyMessageRespVO> pageResult = notifyMessageService.getMyMyNotifyMessagePage(pageVO,
    ...
    @PutMapping("/update-read-only")
    @Operation(summary = "标记站内信为已读")
    @Parameter(name = "id", description = "编号", required = true)
    public CommonResult<Boolean> updateNotifyMessageReadById(@RequestParam("id") Long id) {
        notifyMessageService.updateNotifyMessageRead(ListUtil.of(id), getLoginUserId(), UserTypeEnum.ADMIN.getValue());
        return success(Boolean.TRUE);
    }
