...
public interface StoreProductMapper extends BaseMapperX<StoreProductDO> {
    ...
    default PageResult<StoreProductDO> selectPage(StoreProductPageReqVO reqVO) {
        ...
        wrapper.likeIfPresent(StoreProductDO::getStoreName, reqVO.getStoreName())
                ...
                .eqIfPresent(StoreProductDO::getId, reqVO.getProductId())
                .orderByDesc(StoreProductDO::getId);

        wrapper.eqIfPresent(StoreProductDO::getIsShow,Convert.toInt(reqVO.getIsShow()));
...
//        }
    ...
    @Update("update yshop_store_product set inventory_warning_enable = #{status} where id = #{id}")
     ...
     * @return
     */
    @Update("update yshop_store_product set stock=stock+#{num}" +
            " where id=#{productId}")
