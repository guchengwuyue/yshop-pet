...
public class StoreProductServiceImpl extends ServiceImpl<StoreProductMapper,StoreProductDO> implements StoreProductService {
    ...
    public PageResult<StoreProductRespVO> getStoreProductPage(StoreProductPageReqVO pageReqVO) {
        //如果是条形码搜索
        Long productId = null;
        if (StrUtil.isNotBlank(pageReqVO.getBarCode())) {
            List<StoreProductAttrValueDO> attrs = storeProductAttrValueService.list(new LambdaQueryWrapper<StoreProductAttrValueDO>()
                    .eq(StoreProductAttrValueDO::getBarCode, pageReqVO.getBarCode()));
            //组合商品ID
            if (CollUtil.isNotEmpty(attrs)){
                productId = attrs.get(0).getProductId();
                pageReqVO.setProductId(productId);
            }
        }
        PageResult<StoreProductDO> pageResult = storeProductMapper.selectPage(pageReqVO);
        PageResult<StoreProductRespVO> result = co.yixiang.yshop.framework.common.util.object.BeanUtils.toBean(pageResult, StoreProductRespVO.class);
        //返回商品属性
        result.getList().forEach(item -> {
            List<StoreProductAttrValueDO> attrs = storeProductAttrValueService.list(new LambdaQueryWrapper<StoreProductAttrValueDO>()
                    .eq(StoreProductAttrValueDO::getProductId, item.getId()));
            item.setAttrValueList(attrs);
        });
        return result;
    }
...
//            valueMap.put("integral", 0);
            ...
            if (id > 0) {
                ...
                if (storeProductAttrValue != null) {
                    ...
                    valueMap.put("good_code", storeProductAttrValue.getGoodCode());
...
//                    valueMap.put("integral", storeProductAttrValue.getIntegral());
    ...
    private void addMap(Map<String, Object> headerMap, List<Map<String, Object>> headerMapList, String align, boolean isActivity) {
        ...
        headerMap.put("slot", "good_code");
        headerMap.put("align", align);
        headerMap.put("minWidth", 140);
        headerMapList.add(ObjectUtil.clone(headerMap));

        headerMap.put("title", "产品条码");
...
//        }
    ...
    @Override
    public StoreProductImportRespVO importStoreProductList(List<StoreProductImportExcelVO> importProducts, Long cateId) {
        ...
        // 校验分类存在且可用，导入商品统一归属该分类
        productCategoryService.validateCategory(cateId);
        ...
        StoreProductImportRespVO respVO = StoreProductImportRespVO.builder()
                ...
                .build();
        int rowIndex = 1;
        for (StoreProductImportExcelVO importProduct : importProducts) {
            rowIndex++;
            String storeName = StrUtil.trim(importProduct.getStoreName());
            String rowKey = StrUtil.isNotBlank(storeName) ? storeName : ("第" + rowIndex + "行");
            try {
                if (StrUtil.isBlank(storeName)) {
                    respVO.getFailureNames().put(rowKey, "商品名称不能为空");
                    continue;
                }
                if (importProduct.getPrice() == null) {
                    respVO.getFailureNames().put(rowKey, "售价不能为空");
                    continue;
                }
                insertAndEditYxStoreProduct(buildImportStoreProductDto(importProduct, cateId, storeName));
                respVO.getCreateNames().add(storeName);
            } catch (Exception ex) {
                respVO.getFailureNames().put(rowKey, ex.getMessage());
            }
        }
        return respVO;
    }
    ...
    /**
     * 将导入行转换为单规格商品创建 DTO（统一 specType=0，复用现有新增逻辑）
     */
    private StoreProductDto buildImportStoreProductDto(StoreProductImportExcelVO importProduct, Long cateId, String storeName) {
        String image = StrUtil.blankToDefault(importProduct.getImage(), "");
        String storeInfo = StrUtil.blankToDefault(importProduct.getStoreInfo(), "");
        String description = StrUtil.blankToDefault(storeInfo, storeName);
        Double price = importProduct.getPrice() == null ? 0d : importProduct.getPrice();
        Double cost = importProduct.getCost() == null ? 0d : importProduct.getCost();
        Double otPrice = importProduct.getOtPrice() == null ? price : importProduct.getOtPrice();
        Integer inventoryWarningEnable = importProduct.getInventoryWarningEnable() == null
                ? 0 : importProduct.getInventoryWarningEnable();
        Integer inventoryWarningNumber = importProduct.getInventoryWarningNumber() == null
                ? 0 : importProduct.getInventoryWarningNumber();
        // 模板不再填写商品编码，系统自动生成以满足单规格创建校验
        String goodCode = "SP" + IdUtil.getSnowflakeNextIdStr();

        ProductFormatDto attr = ProductFormatDto.builder()
                .pic(image)
                .price(price)
                .cost(cost)
                .otPrice(otPrice)
        ...
        storeProductDto.setImage(image);
        storeProductDto.setStoreName(storeName);
        storeProductDto.setCateId(String.valueOf(cateId));
        storeProductDto.setUnitName(StrUtil.blankToDefault(importProduct.getUnitName(), "件"));
        storeProductDto.setStoreInfo(storeInfo);
        storeProductDto.setDescription(description);
        storeProductDto.setBarCode("");
        storeProductDto.setSpecType(SpecTypeEnum.TYPE_0.getValue());
        storeProductDto.setIsShow(ShopCommonEnum.SHOW_1.getValue());
        storeProductDto.setInventoryWarningEnable(inventoryWarningEnable);
        storeProductDto.setInventoryWarningNumber(inventoryWarningNumber);
        storeProductDto.setAttrs(ListUtil.toList(attr));
        storeProductDto.setItems(Collections.emptyList());
